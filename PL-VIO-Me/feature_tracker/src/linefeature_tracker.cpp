...
bool visualize_EDline_match(Mat imageMat1, Mat imageMat2,
                          std::vector<LineSegment> octave0_1, std::vector<LineSegment>octave0_2)
{
    //	Mat img_1;
    cv::Mat img1,img2;
    if (imageMat1.channels() != 3){
    ...
    int lowest = 0, highest = 255;
    int range = (highest - lowest) + 1;
    if(octave0_2.size()!=octave0_1.size()){
        ROS_INFO("match error");
        return false;
    }
    for (int k = 0; k < octave0_1.size(); ++k) {
        LineSegment line1 = octave0_1[k];
        LineSegment line2=octave0_2[k];
        //KeyLine line1 = octave0_1[mt.queryIdx];  // trainIdx
        //KeyLine line2 = octave0_2[mt.trainIdx];  //queryIdx
        ...
        //unsigned int g = lowest + int(rand() % range);
        //unsigned int b = lowest + int(rand() % range);
        cv::Point startPoint = cv::Point(int(line1.sx), int(line1.sy));
        cv::Point endPoint = cv::Point(int(line1.ex), int(line1.ey));
        cv::line(img1, startPoint, endPoint, cv::Scalar(0, 255, 0),2 ,8);
        putText(img1,cv::format("%2d",k),startPoint,FONT_HERSHEY_TRIPLEX, 1.0, Scalar(50, 20, 250), 2, 8);
        //cv::addText(img1,cv::format("%2d",k),cv::Scalar(0,0,255),LINE_8);
        //cv::imwrite("/home/seuzsg/vinsdate/matchedmith5/forw"+cv::format("%d",name++)+".jpg",img1);
        cv::Point startPoint2 = cv::Point(int(line2.sx), int(line2.sy));
        cv::Point endPoint2 = cv::Point(int(line2.ex), int(line2.ey));
        cv::line(img2, startPoint2, endPoint2, cv::Scalar(0, 255, 0),2, 8);
        putText(img2,cv::format("%2d",k),startPoint2,FONT_HERSHEY_TRIPLEX, 1.0, Scalar(50, 20, 250), 2, 8);
        //cv::imwrite("/home/seuzsg/vinsdate/matchedmith5/cur"+cv::format("%d",name++)+".jpg",img2);
        //cv::addText(img2,cv::format("%2d",k),cv::Scalar(0,0,255),LINE_8);
        //cv::line(img2, startPoint, startPoint2, cv::Scalar(255, 0, 0),1, 8);
        //cv::line(img2, endPoint, endPoint2, cv::Scalar(255, 0, 0),1, 8);

    }
    //cv::imwrite("/home/seuzsg/vinsdate/matchedmith5/forw"+cv::format("%d",name)+".jpg",img1);
    //cv::imwrite("/home/seuzsg/vinsdate/matchedmith5/cur"+cv::format("%d",name++)+".jpg",img2);
    imshow("EDLINE matches1", img1);
    imshow("EDLINE matches2", img2);
    waitKey(1);
    return true;
}
void LineFeatureTracker::readImage(const cv::Mat &_img)
...
//    cv::waitKey(1);
    ...
    if (forwframe_ == nullptr) // 系统初始化的第一帧图像
    {
        forwframe_.reset(new FrameLines);
        curframe_.reset(new FrameLines);
        preframe_.reset(new FrameLines);
        forwframe_->img = img;
        curframe_->img = img;
        preframe_->img=img;
        first_img = true;
    ...
    ///EDLines contrast lines;
    std::vector<LineSegment> linesegments;
    TicToc t_one;
    EDLines LineTrack(forwframe_->img,1);
    linesegments=LineTrack.EdlineChoose(300);///选取150条线
    sum_time+=t_one.toc();
    ROS_INFO("line _detect cost %fms ",t_one.toc());
        if(first_img)
            for(int i=0;i<150;i++)   {
                allfeature_cnt++;
                forwframe_->EDlineID.push_back(allfeature_cnt);///如果是第一帧，就按照顺序排序
                forwframe_->EDLS.push_back(linesegments[i]);
            }
                
        else{
            forwframe_->EDLS=linesegments;
            for(int i=0;i<forwframe_->EDLS.size();i++)
                forwframe_->EDlineID.push_back(-1);   // give a negative id，默认值为-1,后面更新id
...
///////////////
    ...
    forwframe_->keylsd = keylsd;///将当前帧检测到的线和描述算子给到forwframe
...
/*
    for (size_t i = 0; i < forwframe_->keylsd.size(); ++i) {
        if(first_img)
            forwframe_->lineID.push_back(allfeature_cnt++);///如果是第一帧，就按照顺序排序
        else
            forwframe_->lineID.push_back(-1);   // give a negative id，默认值为-1,后面更新id
    }
*/

/////////EDLines match:
    if(forwframe_->EDLS.size()>0){
    if(curframe_->EDLS.size()>0){
        std::vector<LineSegment> goodmatchlines,newlines,goodmatchlines_cur;
        std::vector<int> goodmatchID,newlinesID;
        std::vector<cv::Point2d> matchSP,matchSP2;
        
        TicToc t_two;
        LineTrack.LienMatch_tree(curframe_->EDLS);
        sum_time+=t_two.toc();
        ROS_INFO("match lines cost %fms ",t_two.toc());
        //FILE *fp=NULL;
        //fp=fopen("/home/seuzsg/ZED_WS/src/time_line.txt","a");
        //fprintf(fp," %f %f\n",t_one.toc(),t_two.toc());
        //fclose(fp);
        ROS_INFO("hahah %fms ",t_one.toc());
        for(int j=0;j<curframe_->EDLS.size();j++){///150条
            for(int i=0;i<forwframe_->EDLS.size();i++){///50条
                if(LineTrack.mapL[i][j]!=0){
                    forwframe_->EDlineID[i]=curframe_->EDlineID[j];
                    
                    goodmatchID.push_back(forwframe_->EDlineID[i]);
                    goodmatchlines.push_back(forwframe_->EDLS[i]);
                    goodmatchlines_cur.push_back(curframe_->EDLS[j]);
                    matchSP.push_back(cv::Point2d(forwframe_->EDLS[i].sx,forwframe_->EDLS[i].sy));
                    matchSP2.push_back(cv::Point2d(curframe_->EDLS[j].sx,curframe_->EDLS[j].sy));
                
                }
            }
        }
        for(int i=0;i<forwframe_->EDLS.size();i++){
            if(forwframe_->EDlineID[i]==-1){
                newlines.push_back(forwframe_->EDLS[i]);
                newlinesID.push_back(forwframe_->EDlineID[i]);
            }
        }
        ///ROS_INFO("there are %d lines, %d new lines after match",goodmatchlines.size(),newlines.size());
        ////ks ransac
        int goodline_size=goodmatchlines.size();
        if(matchSP.size()>10)
        {
            float fx = K_.at<float>(0, 0);
            float fy = K_.at<float>(1, 1);
            float cx = K_.at<float>(0, 2);
            float cy = K_.at<float>(1, 2);
            for (unsigned int i = 0; i <matchSP.size(); i++)
                {
                matchSP[i].x = (matchSP[i].x - cx)/fx;
                matchSP[i].y = (matchSP[i].y - cy)/fy;
                matchSP2[i].x = (matchSP2[i].x - cx)/fx;
                 matchSP2[i].y = (matchSP2[i].y - cy)/fy;
                }
            std::vector<uchar> status;
            cv::findFundamentalMat(matchSP, matchSP2, cv::FM_RANSAC, F_THRESHOLD, 0.99, status);
            int j=0;
            for(int i=0;i<goodmatchlines.size();i++){
                if(status[i]){
                    goodmatchlines[j]=goodmatchlines[i];
                    goodmatchlines_cur[j]=goodmatchlines_cur[j];
                    goodmatchID[j]=goodmatchID[i];
                    j++;
                }
                else{
                    newlines.push_back(goodmatchlines[i]);
                    newlinesID.push_back(-1);
            }
        }
        goodmatchlines.resize(j);
        goodmatchlines_cur.resize(j);

        goodmatchID.resize(j);
        ROS_INFO("there are %d lines, %d new lines after match",goodmatchlines.size(),newlines.size());
        ROS_INFO("there are %d lines are tiked after RANSAC",goodline_size-goodmatchlines.size());
        }
        if(!preframe_->EDlineID.empty()){
            int Threelinecnt=0;
            for(int i=0;i<preframe_->EDlineID.size();i++){
                for(int j=0;j<goodmatchID.size();j++){
                    if(preframe_->EDlineID[i]==goodmatchID[j])
                        Threelinecnt++;
                }
            }
            ROS_INFO("there are %d lines tracked 3frames or more",Threelinecnt);
        }
        if(goodmatchlines_cur.size()!=goodmatchlines.size()){
            ROS_INFO("eroror match");
        }
        if(goodmatchlines.size()>0)
            visualize_EDline_match(forwframe_->img,curframe_->img,goodmatchlines,goodmatchlines_cur);
        if(goodmatchlines.size()<150){
            //int addnum=200-goodmatchlines.size();
            for(int i=0;i<newlinesID.size();i++){
                if(newlinesID[i]==-1){
                    allfeature_cnt++;
                    newlinesID[i]=allfeature_cnt;
                    goodmatchID.push_back(newlinesID[i]);
                    goodmatchlines.push_back(newlines[i]);
                }           
            }
        }
        
        forwframe_->EDLS=goodmatchlines;
        forwframe_->EDlineID=goodmatchID;
        }
    ...
    ///将EDLINE转为季哥的line
        for(int i=0;i<forwframe_->EDLS.size();i++){
            Line l;
            LineSegment temls=forwframe_->EDLS[i];
            l.StartPt=cv::Point2d(temls.sx,temls.sy);
            l.EndPt=cv::Point2d(temls.ex,temls.ey);
            l.length=temls.len;
    ...
    ///如果当前帧没有检测到直线，就不要了，且不发布这一帧;
    else{
        ROS_INFO("there is no line detect ");
        ROS_INFO("there is no line detect ");
        ROS_INFO("there is no line detect ");
        ROS_INFO("there is no line detect ");
...
/*
    if(curframe_->keylsd.size() > 0)///如果不是第一帧，开始进行匹配
    {
...
//        ROS_INFO("lbd_macht costs: %fms", t_match.toc());
        ...
        }///将匹配到的forw帧中的线id赋值为之前的线id
        visualize_line_match(forwframe_->img.clone(), curframe_->img.clone(), forwframe_->keylsd, curframe_->keylsd, good_matches);//显示匹配的线
